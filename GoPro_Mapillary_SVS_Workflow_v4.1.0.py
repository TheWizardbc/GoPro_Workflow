# -------------------------------------------------------------------------
# SCRIPT NAME: GoProMax_Hero_Workflow_v4.1.0.py
# VERSION: 4.1.0
...
class MapillaryApp:
    def __init__(self, master, system_bg_color):
        ...
        master.title("GoPro Max & Hero - Mapillary & SVS Workflow Tool (V4.1.0)")
    ...
    def create_about_tab(self, tab):
        ...
        tk.Label(header_frame, text="Version 4.1.0 (Max 1 & 2 + Hero Edition)", font=('Arial', 10, 'bold'), fg='#666', bg=self.system_bg_color).pack()
        ...
        msg_hero = (
            "New in V4.1.0. This dedicated workflow processes standard GoPro Hero MP4 files directly for Mapillary. "
    ...
    def run_workflow(self):
        ...
        try:
            ...
            if self.settings['RunUpload']:
                ...
                self.log_message(f"\n[STEP {step_counter}/10] Uploading to Mapillary...", 'info') # RENUMMERD /10
                ...
                if temp_mapillary_dir.exists():
                    ...
                    for i, seq_dir in enumerate(seq_dirs, 1):
                        ...
                        self.log_message(f"    -> Uploading {seq_dir.name}...", 'info')
                        upload_args = [self.settings['MapillaryToolsPath'], "upload", "--user_name", self.settings['MapillaryUserName'], str(seq_dir), f"--num_upload_workers={self.settings['MapillaryUploadWorkers']}"]
                        self.run_command(upload_args)
            else:
                 self.log_message(f"\n[STEP {step_counter}/10] Mapillary Upload SKIPPED.", 'info') # RENUMMERD /10
