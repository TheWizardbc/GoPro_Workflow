# -------------------------------------------------------------------------
# SCRIPT NAME: GoProMax_Hero_Workflow_v4.2.4.py
# VERSION: 4.2.4
...
class MapillaryApp:
    def __init__(self, master, system_bg_color):
        ...
        master.title("GoPro Max & Hero - Mapillary & SVS Workflow Tool (V4.2.4)")
        ...
        # --- LOGGING SETUP ---
        # Bepaal de map waar het script/exe draait
        if getattr(sys, 'frozen', False):
            application_path = os.path.dirname(sys.executable)
        else:
            application_path = os.path.dirname(os.path.abspath(__file__))

        # Maak een unieke bestandsnaam met datum en tijd (tot op de seconde)
        # Voorbeeld: Workflow_Log_20251222_194530.txt
        timestamp_filename = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.log_file_path = os.path.join(application_path, f"Workflow_Log_{timestamp_filename}.txt")
        
        # Maak het bestand alvast aan (of leeg het) met een header
        try:
            with open(self.log_file_path, "w", encoding="utf-8") as f:
                f.write(f"--- Workflow Log Started: {timestamp_filename} ---\n")
                f.write(f"--- Script Version: {self.version} ---\n\n")
        except Exception as e:
    ...
    def log_message(self, message, message_type='info'):
        """
        Logs to GUI, Console, AND a Text File.
        Handles formatting and newlines correctly.
        """
        
        # 1. Formatting Logic (Witregels)
        add_newline = False
        ...
        # 3. FILE Logging (Toevoegen aan bestand)
        try:
            # We openen met 'a' (append) zodat we steeds regels toevoegen
    ...
    def create_about_tab(self, tab):
        ...
        tk.Label(header_frame, text="Version 4.2.4 (Max 1 & 2 + Hero Edition)", font=('Arial', 10, 'bold'), fg='#666', bg=self.system_bg_color).pack()
