# -------------------------------------------------------------------------
# SCRIPT NAME: GoProMax_Hero_Workflow_v4.4.1.py
# VERSION: 4.4.1
...
class MapillaryApp:
    def __init__(self, master, system_bg_color):
        ...
        master.title("GoPro Max & Hero - Mapillary & SVS Workflow Tool (V4.4.1)")
        ...
        self.config_file = Path("mapillary_config_v4.json")
    ...
    def log_message(self, message, message_type='info', extended_only=False):
        """
        Logs to GUI, Console, AND Text Files (Optional).
        """
        # 1. Formatting Logic
        add_newline = False
        ...
        # Determine Tag
        tag = 'info'
        if message_type == 'cmd': tag = 'cmd'
        elif message_type == 'error' or 'error' in message.lower() or 'problem' in message.lower() or 'fatal' in message.lower(): tag = 'error'
        elif message_type == 'success' or 'success' in message.lower() or 'complete' in message.lower(): tag = 'success'
        elif message_type == 'warning' or 'warning' in message.lower(): tag = 'warning'

        # --- A. WRITE TO EXTENDED LOG ---
        # 1. GUI 
        if self.log_text_extended:
            if add_newline: self.log_text_extended.insert(tk.END, "\n")
            self.log_text_extended.insert(tk.END, formatted_message, tag)
            self.log_text_extended.see(tk.END)
            try: self.log_text_extended.update_idletasks()
            except: pass
        ...
        if not extended_only:
            if self.log_text:
                if add_newline: self.log_text.insert(tk.END, "\n")
                self.log_text.insert(tk.END, formatted_message, tag)
                self.log_text.see(tk.END)
                try: self.log_text.update_idletasks()
                except: pass
            else:
    ...
    def load_settings(self):
        ...
        if os.path.exists(self.config_file): 
            try:
                ...
                self.nadir_scale_var.set(str(loaded_settings.get('NadirScaleFactor', 0.3333)))
                self.nadir_crf_var.set(str(loaded_settings.get('NadirCRF', 17)))
                self.nadir_qp_var.set(str(loaded_settings.get('NadirQP', 24)))
    ...
    def create_config_tab(self, tab):
        ...
        # ROW 1: Username & Suffix
        tk.Label(gen_frame, text="Mapillary Username:", bg=self.system_bg_color).grid(row=1, column=0, sticky='w', padx=5, pady=5)
        ttk.Entry(gen_frame, textvariable=self.mapillary_user_var).grid(row=1, column=1, sticky='ew', padx=5, pady=5)
        
        tk.Label(gen_frame, text="File Suffix (Optional):", bg=self.system_bg_color).grid(row=1, column=2, sticky='w', padx=15, pady=5)
        ttk.Entry(gen_frame, textvariable=self.file_suffix_var).grid(row=1, column=3, sticky='ew', padx=5, pady=5)
        
        # ROW 2: Sample Distance & Workers
        tk.Label(gen_frame, text="Sample Distance (m):", bg=self.system_bg_color).grid(row=2, column=0, sticky='w', padx=5, pady=5)
        ttk.Entry(gen_frame, textvariable=self.sample_distance_var).grid(row=2, column=1, sticky='ew', padx=5, pady=5)
        
        tk.Label(gen_frame, text="Upload Workers:", bg=self.system_bg_color).grid(row=2, column=2, sticky='w', padx=15, pady=5)
        ttk.Entry(gen_frame, textvariable=self.upload_workers_var).grid(row=2, column=3, sticky='ew', padx=5, pady=5)

        # ROW 3: Save Standard Log 
        tk.Label(gen_frame, text="Save Standard Log (.txt):", anchor='w', bg=self.system_bg_color).grid(row=3, column=0, sticky='w', padx=5, pady=5)
        ttk.Checkbutton(gen_frame, variable=self.save_std_log_var).grid(row=3, column=1, sticky='w', padx=5, pady=5)

        # ROW 4: Save Extended Log
        tk.Label(gen_frame, text="Save Extended Log (.txt):", anchor='w', bg=self.system_bg_color).grid(row=4, column=0, sticky='w', padx=5, pady=5)
        ttk.Checkbutton(gen_frame, variable=self.save_ext_log_var).grid(row=4, column=1, sticky='w', padx=5, pady=5)
        ...
        tk.Label(nadir_frame, text="Video Quality (CRF):", bg=self.system_bg_color).grid(row=2, column=2, sticky='w', padx=15, pady=5)
        crf_entry = ttk.Entry(nadir_frame, textvariable=self.nadir_crf_var)
        crf_entry.grid(row=2, column=3, sticky='ew', padx=5, pady=5)
        ...
        try:
            ...
            create_tooltip(crf_entry, crf_tooltip_text)
        except Exception as e:
            print(f"Warning: Could not create CRF tooltip: {e}")
        ...
        # Check Config button removed - 4.4.1
    ...
    def create_max_workflow_tab(self, tab):
        ...
        self.rb_max_process_upload = ttk.Radiobutton(selection_container, text="Sample, Process and Upload", variable=self.mapillary_actions_var, value=1)
        self.rb_max_process_upload.grid(row=5, column=0, sticky='w', padx=5)
        
        # Option 2: Process Only
        self.rb_max_process_only = ttk.Radiobutton(selection_container, text="Sample & Process Only (No Upload)", variable=self.mapillary_actions_var, value=2)
        self.rb_max_process_only.grid(row=5, column=1, sticky='w', padx=5)

        # Option 3: Upload Only
        self.rb_max_upload_only = ttk.Radiobutton(selection_container, text="Upload Only", variable=self.mapillary_actions_var, value=3)
    ...
    def create_about_tab(self, tab):
        ...
        tk.Label(header_frame, text="Version 4.4.1 (Max 1 & 2 + Hero Edition)", font=('Arial', 10, 'bold'), fg='#666', bg=self.system_bg_color).pack()
    ...
    def run_workflow(self):
        ...
        try:
                    ...
                    for i, file in enumerate(video_source_files, 1):
                        ...
                        if self.settings['RunNadirPatch']:
                            ...
                            if w and h:
                                ...
                                if success:
                                    # --- UPDATED BIT DEPTH LOGIC FOR 8, 10 AND 12 BIT ---
                                    if bit_depth == 12:
                                        pix_fmt = "yuv420p12le"
                                    elif bit_depth == 10:
                                        pix_fmt = "yuv420p10le"
                                    else:
                                        pix_fmt = "yuv420p" # Default to 8-bit
                                    
                                    vf = f"[1:v]scale={w}:{nadir_h}[scaled_patch];[0:v][scaled_patch]overlay=0:{y_pos},format={pix_fmt}[final_out]"
                                    
                                    # === GPU ENCODER LOGIC START ===
            ...
            # --- DEFINE OUTPUT DIRECTORY (FLEXIBLE) ---
            target_root = Path(self.settings['TargetDir'])
            specific_subfolder = target_root / "Mapillary_Output"

            if specific_subfolder.exists():
                temp_mapillary_dir = specific_subfolder
                self.log_message(f"    [INFO] Found standard subfolder: {temp_mapillary_dir.name}", 'info')
            else:
                temp_mapillary_dir = target_root
                self.log_message(f"    [INFO] 'Mapillary_Output' subfolder not found. Using root TargetDir: {target_root.name}", 'info')

            should_run_phase_3 = (self.settings['RunCorePrep'] and self.settings['RunSample']) or self.settings['RunProcess']
            ...
            if should_run_phase_3:
                ...
                if self.settings['RunSample']:
                    ...
                    if not temp_mapillary_dir.exists(): temp_mapillary_dir.mkdir(parents=True)
                    ...
                    if total_files == 0:
                        self.log_message("    [WARNING] No *_gpmf_final.mov files found to sample.", 'warning')
            ...
            if self.settings['RunUpload']:
                ...
                if not temp_mapillary_dir.exists():
                    ...
                    if root_desc_file.exists():
                          ...
                          if exit_code != 0:
                              self.log_message(f"    [ERROR] Upload failed or nothing uploaded. Check logs above.", 'error')
                          else:
                              self.log_message(f"    [SUCCESS] Batch upload process finished.", 'success')

                    # SITUATION 2: Checking individual subfolders
            ...
            # --- 10. FINAL CLEANUP (SKIPPED IF UPLOAD ONLY) ---
            # We use RunCorePrep as a proxy: if that is False (Upload Only), we skip cleanup.
            if self.settings['RunCorePrep']:
                step_counter += 1
                ...
                # A. Remove intermediate .mov files
                ...
                if temp_mapillary_dir.exists() and (self.settings['RunProcess'] or self.settings['RunUpload']):
                    ...
                    for seq_dir in temp_sequence_dirs:
                        ...
                        if '_gpmf_final' in new_name: new_name = new_name.replace('_gpmf_final', '')
                        if new_name.endswith('.mov'): new_name = new_name[:-4] 
                        
                        final_seq_name = new_name 
                        # Try to find NADIR_SUFFIX in globals, else skip
                        ...
                        if final_seq_name != original_name and final_seq_name:
                            ...
                            try:
                                ...
                                self.log_message(f"    [SUCCESS] Renamed '{original_name}' to '{new_path.name}'", 'success')
                            except Exception as e:
                                self.log_message(f"    [ERROR] Rename failed '{original_name}': {e}", 'error')
            
            else:
                 # Logic for Upload Only (Cleanup Skipped)
                 self.log_message("\n[INFO] Cleanup Step SKIPPED (Upload Only mode active).", 'info')
            ...
            # SVS Summary
            if 'svs_fix_success' in locals() and self.settings['RunSVSHeaderFix'] and self.settings.get('CameraModel') == 'Max 2' and svs_fix_success:
                svs_upload_dir = Path(self.settings['TargetDir']) / "SVS_Fixed_Headers"
                if svs_upload_dir.exists() and any(svs_upload_dir.iterdir()):
                     final_summary_message += (f"\n\nStreetview Studio files saved in:\n{svs_upload_dir.name}")
    ...
    def run_hero_workflow_logic(self):
        try:
            ...
            for i, video in enumerate(mp4_files, 1):
                ...
                # Assumption: Output is here
                video_frames_dir = final_target_path 

                # --- STEP 1: SAMPLING (Run only if RunSample is True) ---
                if self.settings['RunSample']:
                    ...
                    actual_sample_subfolder = temp_output_dir / video.name 
                    ...
                    if temp_output_dir != final_target_path:
                        try:
                            ...
                            video_frames_dir = temp_output_dir 

                # --- STEP 3: PROCESSING (Run if True - Now enabled for Upload Only too) ---
                if self.settings['RunProcess']:
                    # Crucial check: If we skipped sampling (Upload Only), the folder MUST exist already.
                    if not video_frames_dir.exists():
                         self.log_message(f"  [WARNING] Cannot process. Directory not found: {video_frames_dir.name} (Did you sample first?)", 'warning')
                         continue
                ...
                if self.settings['RunUpload']:
                    ...
                    # Check for JSON just to be sure
                    if not (video_frames_dir / "mapillary_image_description.json").exists():
                         self.log_message(f"  [ERROR] No description file found. Processing step likely failed.", 'error')
                         continue
