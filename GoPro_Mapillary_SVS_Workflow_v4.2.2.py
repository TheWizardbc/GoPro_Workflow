# -------------------------------------------------------------------------
# SCRIPT NAME: GoProMax_Hero_Workflow_v4.2.2.py
# VERSION: 4.2.2
...
# --- HELPER CLASS FOR TOOLTIPS (STABLE FIX) ---
class ToolTip(object):
    ...
    def showtip(self, text):
        ...
        # Check of widget nog bestaat en zichtbaar is
        try:
...
def create_tooltip(widget, text):
    ...
    def enter(event):
        # Annuleer eventuele vorige timers om dubbele calls te voorkomen
        if toolTip.id:
            widget.after_cancel(toolTip.id)
        # Start een nieuwe timer: Toon tooltip na 500ms
        toolTip.id = widget.after(500, lambda: toolTip.showtip(text))
    
    def leave(event):
        # Annuleer de timer als de muis weggaat VOORDAT de tooltip er is
        if toolTip.id:
            widget.after_cancel(toolTip.id)
            toolTip.id = None
        # Verberg de tooltip als hij al zichtbaar is
        toolTip.hidetip()
        
    widget.bind('<Enter>', enter)
    widget.bind('<Leave>', leave)
    # Extra veiligheid: Verberg ook als er op geklikt wordt
...
class MapillaryApp:
    def __init__(self, master, system_bg_color):
        ...
        master.title("GoPro Max & Hero - Mapillary & SVS Workflow Tool (V4.2.2)")
...
# --- GPU/Performance Variables (NEW) ---
        self.use_gpu_encoding_var = tk.BooleanVar(value=False)
        self.gpu_encoder_var = tk.StringVar(value='auto') # 'auto', 'nvenc', 'qsv'
    ...
    def load_settings(self):
        ...
        if SETTINGS_FILE_PATH.exists(): 
            try:
                ...
                # --- FIX: Update GPU Tkinter variables from JSON ---
    ...
    def save_settings(self):
        ...
        settings_to_save = {
            ...
            'NadirQP': self.settings['NadirQP']  # <--- DEZE REGEL ONTBRAK
        }
    ...
    def create_config_tab(self, tab):
        ...
        if self.logo_image:
            # GEWIJZIGD: Logo met kleinere afmeting (100x100) en bovenaan
        ...
        # --- GPU / Hardware Acceleration Settings (AANGEPAST: Tekst Input & Uitlijning) ---
        gpu_group_frame = ttk.LabelFrame(content_frame, text="GPU Acceleration (Nadir & Max 2 Encoding)", padding="10")
        ...
        # Kolom configuratie voor nette uitlijning (Labels links, Inputs rechts)
        gpu_frame.grid_columnconfigure(0, weight=1) # Label Encoder
        gpu_frame.grid_columnconfigure(1, weight=1) # Input Encoder
        gpu_frame.grid_columnconfigure(2, weight=1) # Label QP
        gpu_frame.grid_columnconfigure(3, weight=1) # Input QP

        # 1. Enable GPU Checkbox (Rij 0 - over de hele breedte)
        ...
        # Zorg dat handmatige tekstinvoer ook wordt opgeslagen bij enter of focus verlies
        ...
        try:
            ...
            # Tooltip voor CRF
            create_tooltip(crf_spin, "Lower = Better Quality. Standard is 17.")
  ...
  #               text="Warning: Increased processing time.", 
  #               fg="red", 
  #               font=('Arial', 8, 'bold'),
  #               bg=self.system_bg_color).pack(side='left', padx=5)
    ...
    def create_about_tab(self, tab):
        ...
        tk.Label(header_frame, text="Version 4.2.2 (Max 1 & 2 + Hero Edition)", font=('Arial', 10, 'bold'), fg='#666', bg=self.system_bg_color).pack()
    ...
    def _get_video_bit_depth(self, path):
        ...
        # ffprobe zoekt naar 'bits_per_raw_sample' en 'pix_fmt' tegelijk als fallback.
        cmd = [
        ...
        # Gebruik de interne run_command om de exit code en output te krijgen
        c, o = self.run_command(cmd)
        
        # Standaard terugval naar 8-bit
        default_depth = 8
        
        if c == 0 and o.strip():
            # FFprobe output kan de velden omdraaien. We controleren beide delen van de CSV-output.
            # Output is CSV: [bits_per_raw_sample],[pix_fmt]
            
            # We filteren lege strings (zoals N/A) uit de parts voor robuustheid.
            parts = [p.strip() for p in o.strip().split(',') if p.strip()] 
            
            for part in parts:
                # 1. Controleer op ruwe bitdiepte (de numerieke waarde)
                if part.isdigit() and int(part) >= 8:
                    return int(part)
                
                # 2. Controleer op '10' in de pixelformat-naam (bijv. yuv420p10le)
                if "10" in part:
                     self.log_message(f"    [INFO] Determined 10-bit depth via pixel format check ({part}).", 'info')
                     return 10
        
        # Fallback voor wanneer beide methoden falen
        self.log_message(f"    [WARNING] Could not determine bit depth via FFprobe (Error Code: {c}). Defaulting to 8-bit.", 'warning')
    ...
    def run_workflow(self):
        ...
        try:
                ...
                if self.settings['RunCorePrep']:
                    for i, file in enumerate(video_source_files, 1):
                        ...
                        if self.settings['RunNadirPatch']:
                            ...
                            if w and h:
                                # 1. Detecteer Bit Depth
                                bit_depth = self._get_video_bit_depth(file)
                                
                                # 2. Nadir Patch Afmetingen
                                ...
                                # 4. Nadir Patch Magick Commands (onveranderd)
                                cmds = [
                                ...
                                # 5. FFmpeg Overlay Command (AANGEPAST)
                                if success:
                                    # ... (Bestaande logica voor bit_depth, pix_fmt, vf blijft) ...
                                    # Output pixel format gebaseerd op bit_depth
                                    pix_fmt = "yuv420p" if bit_depth == 8 else "yuv420p10le"
                                    
                                    # Complex filter: scale, overlay, and set final pixel format
                                    vf = f"[1:v]scale={w}:{nadir_h}[scaled_patch];[0:v][scaled_patch]overlay=0:{y_pos},format={pix_fmt}[final_out]"
                                    
                                    # === GPU ENCODER LOGICA START (NEW) ===
                                    if self.settings['UseGPUEncoding']:
                                        ...
                                        # 1. Bepaal Encoder Naam en Hardwareversnelling Vlaggen
                                        if encoder_choice in ['nvenc', 'auto']:
                                            # NVIDIA NVENC (Standaard keuze voor 'auto')
                                            encoder_name = "hevc_nvenc"
                                            
                                        elif encoder_choice == 'qsv':
                                            # Intel Quick Sync - Vereist '-hwaccel qsv' in de input.
                                            encoder_name = "hevc_qsv"
                                            # FFmpeg heeft vaak een '-hwaccel' vlag nodig vóór de '-i' input voor QSV.
                                            # We voegen de vlaggen toe aan een aparte lijst om later het commando te bouwen.
                                        ...
                                        # 2. Bouw de Encoder Argumenten (Gemeenschappelijk)
                                        
                                        # HIER IS DE WIJZIGING: Gebruik 'NadirQP' in plaats van 'NadirCRF'
                                        qp_val = self.settings.get('NadirQP', 24)
        
                                        encoder_args = [
                                           "-c:v", encoder_name, 
                                           "-qp", str(qp_val),   # Gebruik de aparte QP instelling!
                                           "-tag:v", "hvc1", 
                                           "-c:a", "copy"
                                        ]
                                        # Voor GPU-encoders gebruiken we -q:v (Constant Quantizer) in plaats van -crf.
                    #                    encoder_args = [
                     #                       "-c:v", encoder_name, 
                     #                       "-q:v", str(self.settings['NadirCRF']), 
                     #                       "-tag:v", "hvc1", 
                      #                      "-c:a", "copy"
                      #                  ]
                                        
                                        # 3. Assembleer het volledige commando
                                        # Het QSV-commando moet de hwaccel vlag vroeg in de opdracht hebben.
                                        ...
                                        # === CPU ENCODER LOGICA (FALLBACK/STANDAARD) ===
            ...
            # Strictly separating Max 1 and Max 2 processing logic here.
            # ==============================================================================
            
            svs_fix_success = False
            
            # --- STEP 4: CHANNEL A: MAX 1 MP4 EXPORT (NEW STEP) ---
            if cam_model == 'Max 1':
                step_counter = 4
            ...
            # --- CHANNEL B: MAX 2 PROCESSING (WAS STEP 4/5) ---
            elif cam_model == 'Max 2':
                svs_fix_success = False 
                
                # STEP 5: GPX GENERATION (WAS STEP 4)
                if self.settings['RunCorePrep'] and self.settings['RunGPX']:
                    ...
                    for i, video_file in enumerate(intermediate_mov_files, 1):
                        ...
                        # GPX is placed in SourceDir (conforming to Mapillary standard)
                ...
                # STEP 6: SVS HEADER FIX (WAS STEP 5)
                if self.settings['RunCorePrep'] and self.settings['RunSVSHeaderFix'] and self.settings['RunGPX']:
                    ...
                    # Search for GPX files in SourceDir (created in step 5)
                    ...
                    for i, gpx_path in enumerate(gpx_files_in_source, 1):
                        ...
                        try:
                            ...
                            self.log_message(f"       [SVS FIX] Stripping internal metadata tracks to ensure GPX usage (Original Working Method)...", 'info')
                            cmd_strip = [
                                self.settings['FFmpegPath'], "-y",
                                "-i", str(source_video),
            ...
            if self.settings['RunCorePrep'] and (self.settings['RunSample'] or self.settings['RunProcess']):
                if self.settings['RunSample']:
                    ...
                    for i, video_file in enumerate(intermediate_mov_files, 1):
                        ...
                        # Use Mapillary Tools to sample
                        # Note: We use --video_sample_interval from settings
                        sampling_args = [
                        ...
                        if self.settings['RunNadirPatch'] and self.settings['NadirImagePath']:
                             # Note: Mapillary tools doesn't support direct Nadir patching during sample_video in this version easily.
                             # The Nadir logic is handled via custom FFmpeg processing if implemented separately, 
                             # OR we rely on post-processing. In this workflow, typically we would process the frames after sampling.
                             pass
                ...
                if self.settings['RunProcess']:
                    ...
                    if temp_mapillary_dir.exists():
                        ...
                        # --- NADIR PATCHING (IF ENABLED) ---
                        if self.settings['RunNadirPatch'] and self.settings['NadirImagePath']:
                             self.log_message("    [INFO] Applying Nadir Patch to processed frames (This might take a while)...", 'info')
                             # (Nadir logic here remains unchanged from original, just ensure comments are consistent)
                    else:
            ...
            if self.settings['RunUpload']:
                ...
                # Check if description file is in ROOT DIRECTORY (Your situation)
                ...
                if root_desc_file.exists():
                     ...
                     # Upload temp_mapillary_dir directly, not subfolders
                     upload_args = [
                            self.settings['MapillaryToolsPath'], 
                            "upload", 
                            "--user_name", user_arg, 
            ...
            if temp_mapillary_dir.exists() and (self.settings['RunProcess'] or self.settings['RunUpload']):
                ...
                for seq_dir in temp_sequence_dirs:
                    ...
                    if final_seq_name != original_name and final_seq_name:
                        ...
                        try:
                            # Use rename instead of move for better error handling
            ...
            # Show SVS-ready message ONLY if the SVS-fix was successful in the Max 2 path
            # Note: 'svs_fix_success' is only True if the Max 2 workflow was executed and succeeded
    ...
    def run_hero_workflow_logic(self):
        try:
            ...
            for i, video in enumerate(mp4_files, 1):
                ...
                # Update bar and calculate percentage
                pct = self.update_progress(i, total)
                
                # Show percentage in log
                ...
                if actual_sample_subfolder.exists():
                    ...
                    # Move everything from subfolder one level up
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            # FIX: All normal buttons now orange and bold, with new hover color
            ...
            # FIX: Input fields now white (Default TEntry style)
            ...
            # --- STOP BUTTON STYLE (Red) - CORRECT PLACEMENT ---
